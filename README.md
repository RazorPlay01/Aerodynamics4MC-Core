...
性能实验可通过环境变量临时放大客户端 L2 brick：

```bash
AERO_LBM_SIMULATION_COMPACT=1 AERO_LBM_CLIENT_L2_BRICK_SIZE=128 ./gradlew runClient
```

`/aero_client_l2 status` 会显示实际 `brickSize`、发布 stride、准备预算、step/publish 耗时和 native brick runtime 状态。`AERO_LBM_CLIENT_L2_MAX_ACTIVE_BRICKS`、`AERO_LBM_CLIENT_L2_PUBLISH_INTERVAL_TICKS`、`AERO_LBM_CLIENT_L2_PUBLISH_SAMPLE_STRIDE`、`AERO_LBM_CLIENT_L2_STATIC_BUILD_CELLS_PER_TICK`、`AERO_LBM_CLIENT_L2_COARSE_SEED_CELLS_PER_TICK` 可用于控制测试压力；默认游戏行为仍是 32³。
